...
impl Games{
    ...
    fn send_color_fill_vector_data(&mut self, vector: Vec<String>, addr: Addr<MyWs>){
        for color_data in vector{
            addr.do_send(ServerMsg{data: color_data.to_string()}); 
        }
    }
                ...
                if result{
                    for session in &game.1.sessions{
                        if session.name == msg.name{
                            ...
                            game.1.color_fill_vector.push(
                                format!("/moved {} {} {}", msg.x, msg.y, session.color));
    ...
    fn handle(&mut self, msg: ConnectToGame, _ctx: &mut Context<Self>) -> Self::Result{
        ...
        for game in &mut self.game_list{
            if game.0 == msg.gameid && game.1.sessions.len() < game.1.maxplayers as usize{
                ...
                let color = game.1.gen_color();
                let color_fill_vector = game.1.color_fill_vector.clone();
                ...
                self.send_color_fill_vector_data(color_fill_vector, msg.addr.clone());
